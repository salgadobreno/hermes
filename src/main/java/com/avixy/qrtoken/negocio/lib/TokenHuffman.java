...
public class TokenHuffman {
    ...
    public static final String ARG_BIN = "0011010010";
    ...
    static Map<String, String> codeDictionary = new HashMap<>();
    static SortedSet<String> alphabetSortedDict = new TreeSet<>();
    static Comparator<String> inverseLengthComparator = (o1, o2) -> {
    ...
    static {
        ...
        for (Map.Entry<String, String> keyValue : dictionaryCode.entrySet()) {
            codeDictionary.put(keyValue.getValue(), keyValue.getKey());
        }
...
//                    encodedText += s;
    ...
    public String decode(String binary, int length) {
        ...
        while (decoded.length() < length) {
            int currHuffmanLength = MIN_HUFFMAN_LENGTH;

            while (currHuffmanLength <= MAX_HUFFMAN_LENGTH) {
                String code = binary.substring(0, currHuffmanLength); //java.lang.StringIndexOutOfBoundsException: String index out of range: 7

                if (codeDictionary.get(code) != null) {
    ...
    public String getDecodedBinary() {
